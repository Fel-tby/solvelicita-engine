...
def test_validar_uf_all_legado_usa_nordeste_oficial():
    ...
    assert ufs == pipeline.ALL_UFS_NORDESTE
    assert all_legado is True


def test_validar_uf_all_real_usa_nordeste_oficial():
    ufs, all_legado = pipeline.validar_uf_all(
        "incremental",
        {"collect", "dbt", "process", "score", "sync"},
        ["cauc", "siconfi"],
...
def test_main_all_collect_real_usa_collectors_e_nao_chama_prompt(monkeypatch):
    ...
    assert ("collect_all", "incremental", pipeline.ALL_UFS_NORDESTE, ["cauc"]) in chamadas
...
def test_main_all_collect_sem_collectors_permanece_legado(monkeypatch):
    ...
    assert ("collect_legado", pipeline.ALL_UFS_NORDESTE) in chamadas
